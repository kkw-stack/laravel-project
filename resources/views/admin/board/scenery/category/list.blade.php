...
@section('content')
    <div class="row align-items-center justify-content-between mb-4">
        <div class="col-auto">
            <h1 class="page-title fs-4">메덩골 풍경 카테고리 관리</h1>
        </div><!-- .col -->
    </div><!-- .row -->

    @load_partials('admin.partials.locale-menu', [...compact('locale'), 'route' => 'admin.scenery.category.list'])

    <div class="tab-content position-relative">
        <div class="tab-pane show active" id="langKorea">
            <a href="{{ route('admin.scenery.category.create', compact('locale')) }}" class="btn btn-primary position-absolute end-0 tab-outside-btn">카테고리 등록</a>
                                        ...
                                        <tbody class="jt-handle-sort">
                                            @forelse($categories as $category)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a class="text-decoration-underline" href="{{ route('admin.scenery.category.detail', array_merge(request()->query(), compact('category'))) }}">
                                                            {{ $category->name }}
                                                        </a>
                                                    </td>
                                                    <td>
