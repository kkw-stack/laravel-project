...
@section('content')
                        ...
                        <div class="row align-items-center justify-content-between">
                            <div class="col-auto">
                                <div class="d-flex gap-1">
                                    ...
                                    <a href="{{ route('admin.faq.category.list', request()->query()) }}" class="btn btn-light" id="postCancel">취소</a>
                                </div>
                            </div><!-- .col -->
    ...
    @if(isset($category) && $category->faqs->count() === 0)
        <form id="formDelete" method="post" action="{{ route('admin.faq.category.delete', compact('category'))}}">
            @csrf
            @method('DELETE')
        </form>
    @endif
...
$(function () {
    ...
    $('#postDelete').on('click', function () {
        if ($('#formDelete').length > 0) {
            ...
            alert('{{ __('jt.AL-05', ['count' => $category->faqs->count()]) }}');
        }
    });
});
</script>
