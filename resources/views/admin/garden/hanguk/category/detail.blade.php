...
@section('content')
    <div class="row align-items-center justify-content-between mb-4">
        <div class="col-auto">
            <h1 class="page-title fs-4">한국정원 카테고리 상세</h1>
        </div><!-- .col -->
    </div><!-- .row -->
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input
                                type="text"
                                id="boardTitle"
                                placeholder="카테고리명을 입력해주세요. (공백포함, 30자 이내)"
                                maxlength="30"
                                name="title"
                                @class([
                                    'form-control',
                        ...
                        <div class="mb-0">
                            <label for="boardContent" class="form-label">추가 설명 <em>*</em></label>
                            <textarea
                                id="boardContent"
                                rows="4"
                                name="content"
                                placeholder="추가 설명을 입력해주세요. (공백포함, 500자 이내)"
                                maxlength="500"
                                @class([
                                    'form-control',
                        ...
                        <div class="row align-items-center justify-content-between">
                            <div class="col-auto">
                                <div class="d-flex gap-1">
                                    ...
                                    <a href="{{ route('admin.garden.hanguk.category.list', request()->query()) }}" class="btn btn-light" id="postCancel">취소</a>
                                </div>
                            </div><!-- .col -->
    ...
    @if(isset($category) && $category->koreaGardens->count() === 0)
        <form id="formDelete" method="post" action="{{ route('admin.garden.hanguk.category.delete', compact('category'))}}">
            @csrf
            @method('DELETE')
        </form>
    @endif
...
$(function () {
    ...
    $('#postDelete').on('click', function () {
        if ($('#formDelete').length > 0) {
            ...
            alert('{{ __('jt.AL-05', ['count' => $category->koreaGardens->count()]) }}');
        }
    });
});
</script>
