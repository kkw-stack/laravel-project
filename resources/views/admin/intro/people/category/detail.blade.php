...
@section('content')
    <div class="row align-items-center justify-content-between mb-4">
        <div class="col-auto">
            <h1 class="page-title fs-4">함께한 사람들 카테고리 상세</h1>
        </div><!-- .col -->
    </div><!-- .row -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-0">
                            ...
                            <textarea
                                ...
                                rows="4"
                                name="content"
                                placeholder="추가 설명을 입력해주세요. (공백포함, 10자 이내)"
                                maxlength="10"
                                @class([
                                    'form-control',
                        ...
                        <div class="row align-items-center justify-content-between">
                            <div class="col-auto">
                                <div class="d-flex gap-1">
                                    ...
                                    <a href="{{ route('admin.intro.people.category.list', request()->query()) }}" class="btn btn-light" id="postCancel">취소</a>
                                </div>
                            </div><!-- .col -->
    ...
    @if(isset($category) && $category->people->count() === 0)
        <form id="formDelete" method="post" action="{{ route('admin.intro.people.category.delete', compact('category'))}}">
            @csrf
            @method('DELETE')
        </form>
    @endif
...
$(function () {
    ...
    $('#postDelete').on('click', function () {
        if ($('#formDelete').length > 0) {
            ...
            alert('{{ __('jt.AL-05', ['count' => $category->people->count()]) }}');
        }
    });
});
</script>
