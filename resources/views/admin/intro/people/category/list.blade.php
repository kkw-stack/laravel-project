...
@section('content')
    <div class="row align-items-center justify-content-between mb-4">
        <div class="col-auto">
            <h1 class="page-title fs-4">함께한 사람들 카테고리 관리</h1>
        </div><!-- .col -->
    </div><!-- .row -->

    @load_partials('admin.partials.locale-menu', [...compact('locale'), 'route' => 'admin.intro.people.category.list'])

    <div class="tab-content position-relative">
        <div class="tab-pane show active" id="langKorea">
            <a href="{{ route('admin.intro.people.category.create', compact('locale')) }}" class="btn btn-primary position-absolute end-0 tab-outside-btn">카테고리 등록</a>
                                        ...
                                        <tbody class="jt-handle-sort">
                                                @forelse($categories as $category)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <a class="text-decoration-underline" href="{{ route('admin.intro.people.category.detail', array_merge(request()->query(), compact('category'))) }}">
                                                                {{ $category->title }}
                                                            </a>
                                                        </td>
