...
@section('content')
    <div class="row align-items-center justify-content-between mb-4">
        <div class="col-auto">
            <h1 class="page-title fs-4">팝업</h1>
        </div><!-- .col -->
    </div><!-- .row -->

    @load_partials('admin.partials.locale-menu', [...compact('locale'), 'route' => 'admin.main.popup.list'])

    <div class="tab-content position-relative">
        <div class="tab-pane show active" id="langKorea">
            <a href="{{ route('admin.main.popup.create', compact('locale')) }}" class="btn btn-primary position-absolute end-0 tab-outside-btn">등록</a>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered">
                                        ...
                                        <tbody class="jt-handle-sort">
                                            @forelse($sortPopups as $popup)
                                                <tr>
                                                    <input type="hidden" name="sort_ids[]" value="{{ $popup->id }}" />
                                                    <td class="jt-handle-sort__grap text-center fs-5"><i class="mdi mdi-menu"></i></td>
                                                    <td>{{ $popup->status ? '공개' : '비공개' }}</td>
                                                    <td>
                                                        <a class="text-decoration-underline" href="{{ route('admin.main.popup.detail', array_merge(request()->query(), compact('popup'))) }}">{{ $popup->title }}</a>
                                                    </td>
                                                    <td>{{ $popup->author->name }}</td>
                ...
                <div class="col-md-12 stretch-card">
                    <div class="card">
                        ...
                        <div class="card-body">
                                    ...
                                    <tbody>
                                        @forelse($popups as $popup)
                                            <tr>
                                                <td>{{ $popup->status ? '공개' : '비공개' }}</td>
                                                <td>
                                                    <a class="text-decoration-underline" href="{{ route('admin.main.popup.detail', array_merge(request()->query(), compact('popup'))) }}">
                                                        {{ $popup->title }}
                                                    </a>
                                                </td>
                                                <td>{{ $popup->author->name }}</td>
                            ...
                            {{ $popups->withQueryString()->links('admin.partials.pagination') }}
                        </div><!-- .card-body -->
                    </div><!-- .card -->
                </div><!-- .col -->
            </div><!-- .row -->
